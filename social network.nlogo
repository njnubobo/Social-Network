;; For build four kinds networks
;; fully-connected network
;; small-world network
;; BA scale-free network
;; PA network with alternative alpha
...
links-own [
  rewired?                               ;; for building small-world network
]
...
to agent-build
  ...
  ;; build network according to 'network-type'
  ...
  [ifelse network-type = "BA-scale-free" [
      build-BA-network
  ]
  [ifelse network-type = "preferential-attachment" [
      build-BA-network
  ]
...
to evolution
  ask turtles [
    ...
    let pre-selected other (turtle-set [link-neighbors] of neighbs)
    let pre-selection turtles

    let node [who] of self
    let list-neighbs [who] of neighbs
    let list-pre-selected [who] of pre-selected
    let list-pre-selection list-pre-selected
    let list-all [who] of turtles

    ifelse count pre-selected < defined-size [

      ifelse count turtles - 1 - count neighbs > defined-size [
        let list-left (filter [not member? ? list-pre-selected] (filter [not member? ? list-neighbs] (filter [not member? ? (list node)] list-all)))
        set list-pre-selection sentence list-pre-selected n-of (defined-size - count pre-selected) list-left
      ]
      [
        set list-pre-selection (filter [not member? ? list-neighbs] (filter [not member? ? (list node)] list-all))
        ]
    ]
    [set list-pre-selection n-of defined-size list-pre-selected]

    let max-degree 0
    let max-node first list-pre-selection
    foreach list-pre-selection [
      let degree count [link-neighbors] of turtle ?
      if degree > max-degree [
        set max-degree degree
        set max-node ?
      ]
    ]
    create-link-with turtle max-node [
      set color red
    ]
...
to transitive-linking
  ask one-of turtles [
    ifelse count link-neighbors > 1 [
      let neighb-list [who] of n-of 2 link-neighbors
      let node1 first neighb-list
      let node2 last neighb-list
      if link node1 node2 = nobody [
        ask turtle node1 [
          create-link-with turtle node2
        ]
      ]
      ]
    [let another one-of other turtles
     if not link-neighbor? another [
       create-link-with another
     ]
    ]
    ]
  ...
  let kill? false
  ask one-of turtles [
    if random-float 1 < removing-probability [
      ask my-links [die]
      set kill? true
      die
    ]
  ...
  let success? do-calculations    ;; update the clustering-coefficient and average-path-length
  layout
  layout
...
to build-random-network
  ...
  let list-all sort [who] of turtles
  foreach list-all [
    let node1 ?
    let list-bigger filter [? > node1] list-all
    foreach list-bigger [
      let node2 ?
      if link node1 node2 = nobody [
        if random-float 1 < P [
...
to build-fully-connected
  ;; adapted from fully-connected network
  create-turtles number [
    ;;init-agents
    set color white
  ]
...
to build-small-world
  ;; adapted from small-world network
      create-turtles number [
        set color white
      ]
    ...
    ask links [
      ...
      [
        ...
        [
          ...
          ;; wire the new edge
          ask node1 [ create-link-with node2 [ set color gray  set rewired? true ] ]
  ...
  [
    ...
    if old-node != nobody
      [ create-link-with old-node [ set color gray ]
        ...
        if network-type = "BA-scale-free" [
          if TC? [
            let second-node one-of other [link-neighbors] of old-node
            if second-node != nobody [
...
to-report find-partner
  ifelse network-type = "BA-scale-free" [
    report [one-of both-ends] of one-of links           ;; BA-scale-free network
  ]
  [ifelse network-type = "preferential-attachment" [    ;; PA network with changeable alpha

      let tmp map [? ^ alpha] [count my-links] of turtles
      let all-lattice sum tmp
      let node one-of turtles

      let selected? false
      while [not selected?]
      [
        let tmp2 [count my-links] of node ^ alpha
        ifelse random-float 1 < tmp2 / all-lattice [
          set selected? true

        ]
        [
          set node one-of turtles
        ]
      ]
      report node
      ]
  [report one-of turtles                                ;; randomly choose one from present nodes
    ]]
end
...
to-report limit-magnitude [number1 limit1]
  if number1 > limit1 [ report limit1 ]
  if number1 < (- limit1) [ report (- limit1) ]
  report number1
end
...
to find-path-lengths
  ...
  let all-list sort [who] of turtles
  ...
  [
    ...
    while [j < node-count]
    [
      set node1 turtle item i all-list
      set node2 turtle item j all-list
  ...
  [
    ...
    [
      ...
      while [j < node-count]
      [
        ;; network-typenate path length through kth node
        set dummy ( (item k [distance-from-other-turtles] of turtle (item i all-list)) +
                    (item j [distance-from-other-turtles] of turtle (item k all-list)))
        ;; is the network-typenate path shorter?
        if dummy < (item j [distance-from-other-turtles] of turtle (item i all-list))
        [
          ask turtle (item i all-list) [
...
447
461
when building ,each time a k-degree node is selected with P = K ^ alpha\n
15
0.0
1
...
153
655
each time a new node with one link is added
15
0.0
1
...
441
647
to increase the clustering coefficient
15
0.0
1
